                        ...
                        if dist < 2 then
                            if PlayerData.job.name == Config.ClothingRooms[k].requiredJob then
                                ...
                                if IsControlJustPressed(0, 38) then -- E
                                    ...
                                    gender = "male"
                                    if ESX.GetPlayerData().sex == "m" then gender = "female" end
                                ...
                                if PlayerData.gang.name == Config.ClothingRooms[k].requiredJob then
                                    ...
                                    if IsControlJustPressed(0, 38) then -- E
                                        customCamLocation = Config.ClothingRooms[k].cameraLocation
                                        gender = "male"
                                        if ESX.GetPlayerData().sex == "f" then gender = "female" end
                                        ESX.TriggerServerCallback('qb-clothing:server:getOutfits', function(result)
                                            openMenu({
...
RegisterNUICallback('TrackerError', function()
    ESX.ShowNotification("Bunu Çıkaramazsın!")
end)
...
RegisterNUICallback('removeOutfit', function(data, cb)
    ...
    ESX.ShowNotification(data.outfitName..' adlı kıyafetini sildin!')
end)
...
AddEventHandler('qb-clothing:client:loadOutfit', function(oData)
    ...
    if oData.outfitName ~= nil then
        ESX.ShowNotification(oData.outfitName..' adlı kıyafetini onaylaya basarak kaydet')
    end
end)
